...
1. **Parse** (`parser.rs`) — splits the file on **column-0** `###` markers into `RequestBlock`s (an indented `###` is body content), captures `@name = value` in-place variables from every section preamble, and records 1-based `SourceRange` for each block so `--line` can map cursor position back to a request. The region before the first `###` is a section too, so a file may open with a bare request. A request line may carry a trailing `HTTP/x.y` token, which is stripped. Section preambles can carry `# @timeout`, `# @connection-timeout`, `# @no-redirect` directives (→ `RequestOptions`), plus `# @expect-*` assertions (→ `ResponseAssertion`) and `# @capture` directives (→ `CaptureDirective`). Bodies are an enum: `RequestBody::Inline(String)` or `RequestBody::FromFile { path }` when the body is a single `< ./path` line — the space after `<` is required, so `<hello/>` is an inline XML body. Trailing `>> path` / `>>! path` lines (last `>>` wins) become a `ResponseRedirect`. IntelliJ's `# @name` directive lands in `RequestBlock::name_directive` and takes precedence over the `###` separator text (see `RequestBlock::resolved_name`), though `--name` matches either. Unknown `# @directive` names have no effect on execution, for forward compatibility, but are preserved verbatim in `RequestBlock::unknown_directives` so `format` re-emits rather than deletes them.
...
3. **Resolve variables** (`env.rs` + `dynamic.rs` + `interpolate.rs`) — `load_environment` walks from the request file's directory upward (stopping at `--worktree`, which is canonicalised first so a relative or symlinked root still bounds the walk) discovering `http-client.env.json` (public) and `http-client.private.env.json` (private). Layering, lowest-to-highest precedence: dynamic vars (`build_dynamic_variables`: `$uuid`, `$timestamp`, `$isoTimestamp`, `$randomInt`) → public env → private env → in-file `@vars` → `--var` CLI overrides → `run-all` captures from earlier requests. `resolve_variables` then expands `{{var}}` references *within* values (multi-pass, bounded so a self-referential value can't grow without limit), and `interpolate_text` substitutes into the URL, headers, body, and response-redirect path. Unknown names are a hard error there, including one carried in by a substituted value — the interpolated output is rescanned so a typo inside an env value can never send literal `{{braces}}` to the server.
...
Three importers convert external formats into a `RequestFile` for the formatter (they never send anything): `postman.rs` (Postman v2.1 JSON), `har.rs` (HAR 1.2 archives, gzip-aware via `flate2`), and `curl.rs` (a single "Copy as cURL" command, including bundled short options like `-XPOST` and bash ANSI-C `$'...'` bodies). Because importer input is external data, the CLI renders through `format_request_file_checked`, which re-parses its own output and refuses to write a file whose names or bodies would read back as something else.

The GraphQL schema cache is keyed by scheme+host+port+path (`schema_slug`), so two services on different ports of one host don't collide; the pre-0.5 host-only filename is still read as a fallback.
